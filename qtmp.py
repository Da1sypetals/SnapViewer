...
class SnapViewerApp(QMainWindow):
    ...
    def __init__(self, args, message_queue):
        ...
        # Setup timer to check the message queue
...
def run_gui(args, message_queue):
    ...
    app_instance = SnapViewerApp(args, message_queue)
    app_instance.show()
...
def main():
    ...
    # Create a callback function that puts messages into the queue
    ...
    gui_process = multiprocessing.Process(target=run_gui, args=(args, message_queue), daemon=True)
    gui_process.start()
